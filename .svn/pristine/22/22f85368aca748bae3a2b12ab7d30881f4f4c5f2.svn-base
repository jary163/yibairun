...
public abstract class AChatActivity extends ActivitySupport {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		to = SystemUtils.getUserNameToLocal(getIntent().getStringExtra("to"));
		if (to == null)
			return;
		try {
			chat = XmppConnectionManager.getInstance().getConnection()
					.getChatManager().createChat(SystemUtils.getUserNameToService(context, to), null);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private BroadcastReceiver receiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (Constant.NEW_MESSAGE_ACTION.equals(action)) {
				...
				if(to.equals(message.getFromSubJid())){
