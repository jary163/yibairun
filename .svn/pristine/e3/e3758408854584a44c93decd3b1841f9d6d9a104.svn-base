 ...
 * @author shimiso
 */
public class IMContactService extends BaseServie {
	...
	private RosterListener rosterListener = new RosterListener() {
		...
		public void entriesUpdated(Collection<String> addresses) {
			for (String address : addresses) {
				...
				if (ContacterManager.contacters.get(address) != null) {
					// 这里发布的是更新前的user
					intent.putExtra(User.userKey,
							(Parcelable) ContacterManager.contacters
									.get(address));
		...
		public void entriesDeleted(Collection<String> addresses) {
			for (String address : addresses) {
				...
				intent.putExtra(User.userKey, (Parcelable) user);
				sendBroadcast(intent);
			}
		}
