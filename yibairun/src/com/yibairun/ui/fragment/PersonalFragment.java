...
public class PersonalFragment extends BaseFragment implements ScrollTabHolder, ViewPager.OnPageChangeListener {
    ...
    private View ll_accrued_income;
    private View rl_people_num;
    private View ll_accumulative_total;
...
//            context.getTheme().resolveAttribute(R.attr.actionBarSize, mTypedValue, true);//换为v7再打开
    ...
    private void initView() {
        ...
        ll_accrued_income =  rootView.findViewById(R.id.ll_accrued_income);
        rl_people_num =  rootView.findViewById(R.id.rl_people_num);
        ll_accumulative_total =  rootView.findViewById(R.id.ll_accumulative_total);
...
//        tabs.setTabsValue(dm);
    ...
    private void initDate() {
        ...
        tv_balance.setText(ProductUtils.getFormatNum(user.getMoney()));
        tv_expected_return.setText(ProductUtils.getFormatNum(user.getUserinvest()));
        tv_inveting.setText(ProductUtils.getFormatNum(user.getInvestmoney()));
        tv_accrued_income.setText(ProductUtils.getFormatNum(user.getBonusmoney()));
        tv_recommend_nums.setText(String.valueOf(user.getFritotal()));
        tv_accumulative_total.setText(ProductUtils.getFormatNum(user.getRebatemoney()));
...
//            ll_accrued_income.setVisibility(View.GONE);
            rl_people_num.setVisibility(View.GONE);
            ll_accumulative_total.setVisibility(View.GONE);
        }
    }
...
//		mQueue.add(imageRequest);
        RequestManager.addToRequestQueue(imageRequest);
    ...
    protected Response.ErrorListener createMyReqErrorListener() {
        return new Response.ErrorListener() {
            ...
            public void onErrorResponse(VolleyError error) {
                ...
                //if(!TextUtils.isEmpty(appController.getAppKey())){
                  String msg =  VolleyErrorHelper.getMessage(error, context);
                    if(context.getResources().getString(R.string.login_time_out).equals(msg)){//仅超时才让用户强退
    ...
    class TabPageIndicatorAdapter extends FragmentPagerAdapter {
        ...
        public Fragment getItem(int position) {
        	...
        	if(position==0){
        		fragment =  InvestFragment.newInstance(position,InvestType.INVESTING);
        	}else{
        		fragment =  InvestFragment.newInstance(position,InvestType.INVESTED);////目前好像没有数据，用投资中的数据来测试
        	}
