...
public class RecommendFragment extends BaseFragment implements RecommendCallBackListener{
    ...
    private ImageView iv_recommend_quickmark;
    private String mCodeUrl = "http://www.100run.com/api/qrcode.html?appkey=%1$s";
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        iv_recommend_quickmark = (ImageView)rootView.findViewById(R.id.iv_recommend_quickmark);
        iv_recommend_quickmark.setOnClickListener(this);
        return rootView;
    }

	private void initShareUrl() {
        if(!TextUtils.isEmpty(appController.getAppKey())) {//判断是否存在邀请码
//        User user= appController.getUserinfo().getUser();
            ImageCacheManager.getInstance().getImage(String.format(mCodeUrl,appController.getAppKey()), ImageLoader.getImageListener(iv_recommend_quickmark, R.drawable.ic_barcode, R.drawable.ic_barcode));
        }
