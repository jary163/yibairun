...
public class FileUtil {
	 ...
	 * @return
	 */
	public static List<String> getFormatFiles(String path){
		List<String> files = new ArrayList<String>();
		File[] dirFiles = getDirFiles(path);
		for (File file : dirFiles) {
			String fileName = file.getName();
			if(!fileName.contains("journal")){//说明是正常的聊天数据库文件
				files.add(fileName);
			}
		}
		return files;
    ...
    public static Drawable getImageFromAssetsToDrawable(Context context, String fileName) {
    	 try {
             // get input stream
             InputStream ims = context.getResources().getAssets().open(fileName);
    ...
    public static int  genatorFileName(Context context,String path){
    	
    	return (new Random().nextInt(getFileCount(context,path)))+1;	
    }
    	public static int getFileCount(Context context,String path){
    	try {
			return context.getAssets().list(path).length;
		} catch (IOException e) {
